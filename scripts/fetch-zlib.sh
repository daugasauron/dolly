...
commit="${DOLLY_ZLIB_COMMIT}"
source_dir="${project_dir}/.cache/zlib-${commit}"
url="${DOLLY_ZLIB_URL}"

mkdir -p "${project_dir}/.cache"
if [[ ! -d "${source_dir}/.git" ]]; then
  if [[ -e "${source_dir}" ]]; then
    echo "dolly: ${source_dir} exists but is not a zlib checkout" >&2
    exit 1
  fi
  temporary="$(mktemp -d "${project_dir}/.cache/zlib-fetch.XXXXXX")"
