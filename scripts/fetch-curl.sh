...
commit="${DOLLY_CURL_COMMIT}"
source_dir="${project_dir}/.cache/curl-${commit}"
url="${DOLLY_CURL_URL}"
...
if [[ ! -d "${source_dir}/.git" ]]; then
  if [[ -e "${source_dir}" ]]; then
    echo "dolly: ${source_dir} exists but is not a curl checkout" >&2
    exit 1
  fi
  temporary="$(mktemp -d "${project_dir}/.cache/curl-fetch.XXXXXX")"
