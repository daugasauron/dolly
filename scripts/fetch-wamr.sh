...
if [[ ! -d "${source_dir}/.git" ]]; then
  if [[ -e "${source_dir}" ]]; then
    echo "dolly: ${source_dir} exists but is not a WAMR checkout" >&2
    exit 1
  fi
  temporary="$(mktemp -d "${project_dir}/.cache/wamr-fetch.XXXXXX")"
  trap 'rm -rf -- "${temporary}"' EXIT
  git init --quiet "${temporary}"
  git -C "${temporary}" remote add origin "${DOLLY_WAMR_URL}"
...
if [[ "${actual}" != "${commit}" ]]; then
  echo "dolly: expected WAMR ${commit}, found ${actual}" >&2
  exit 1
fi
if ! git -C "${source_dir}" diff --quiet --exit-code; then
  echo "dolly: pinned WAMR checkout has local source changes" >&2
  exit 1
fi
