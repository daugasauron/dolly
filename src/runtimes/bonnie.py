...
def build(sdist_path: str, wheel_path: str) -> None:
    ...
    log_path = "/tmp/bonnie-last-build.log"
        ...
        if status != 0:
            try:
                with open(log_path, "r", encoding="utf-8", errors="replace") as log:
                    lines = log.readlines()
                needles = (
                    "dolly-cc", " error:", "fatal:", "unsupported", "_speedups",
                    "command", "linker", "undefined", "not found", "no such file",
                )
                diagnostic = [
                ...
                if diagnostic:
                    print("bonnie: captured build diagnostic:", file=sys.stderr)
