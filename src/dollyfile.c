...
static int finish_exports(Scope *exports, const Scope *visible, int execute) {
  for (size_t index = 0; index < exports->count; ++index) {
    ...
    if (provider != NULL) {
      Scope copy = {0};
      const int result = scope_add_object(&copy, provider);
      if (result != 0) return result;
      dispose_object(object);
      *object = copy.items[0];
      free(copy.items);
...
static int execute_recipe(Engine *engine, const char *locator,
  ...
  if (result == 0) result = finish_exports(exports_out, &visible, execute);
  if (result == 0 && root) {
