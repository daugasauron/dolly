...
static void usage(FILE *stream) {
  fputs(
      ...
      "standard wheel data, and compiled console scripts are supported.\n",
      stream);
}
...
static int write_entry_source(const char *path, const EntryPoint *entry) {
  FILE *output = fopen(path, "w");
  if (output == NULL) return -1;
  const int result = fprintf(
      output,
      "#include <dolly/runtime.h>\n"
      "static const char code[] = \"import importlib,operator,sys;\"\n"
      "  \"m=importlib.import_module('%s');\"\n"
      "  \"f=operator.attrgetter('%s')(m);\"\n"
      "  \"sys.argv[0]='%s';raise SystemExit(f())\";\n"
      "int main(int argc, char **argv) {\n"
      "  char *arguments[argc + 3];\n"
      "  arguments[0] = \"/usr/bin/python\";\n"
      "  arguments[1] = \"-c\";\n"
      "  arguments[2] = (char *)code;\n"
      "  for (int index = 1; index < argc; ++index) arguments[index + 2] = argv[index];\n"
      "  arguments[argc + 2] = 0;\n"
      "  const int pid = dolly_spawn(\"/usr/bin/python\", argc + 2, arguments, 0, 1, 2);\n"
      "  if (pid < 0) return 126;\n"
      "  int status = 1;\n"
      "  return dolly_wait(pid, &status) == 0 ? status : 1;\n"
      "}\n",
      entry->module, entry->function, entry->name) < 0 ? -1 : 0;
...
static int compile_entry_points(Plan *plans, size_t plan_count,
                                const char *target) {
  unsigned sequence = 0;
  for (size_t plan_index = 0; plan_index < plan_count; ++plan_index) {
    ...
    for (size_t entry_index = 0; entry_index < plan->entry_point_count;
      ...
      char source[96];
      char staged[96];
      snprintf(source, sizeof(source), "/tmp/bonnie-entry-%u.c", ++sequence);
      snprintf(staged, sizeof(staged), "/tmp/bonnie-entry-%u.wasm", sequence);
      remove(source);
      remove(staged);
      if (write_entry_source(source, entry) != 0) return -1;
      char *arguments[] = {
          "/bin/cc", "-O2", "-std=c17", source, "-o", staged, NULL,
      };
      const int pid = dolly_spawn("/bin/cc", 6, arguments,
                                  STDIN_FILENO, STDOUT_FILENO, STDERR_FILENO);
      int status = 1;
      if (pid < 0 || dolly_wait(pid, &status) != 0 || status != 0) {
        fprintf(stderr, "bonnie: could not compile console script %s\n",
                entry->name);
        remove(source);
        remove(staged);
        return -1;
...
static int prepare_resolved_plan(size_t index, Plan *plans, size_t plan_count,
  ...
  if (compile_entry_points(plan, 1, stage_target) != 0) {
