...
import { DEFAULT_LOCAL_MODEL } from "../../src/local-model-contract.mjs";

export async function runStudioModelProof({ evaluate, wait, submit, press }) {
  ...
  await evaluate(`document.querySelector('#local-model [data-model="${DEFAULT_LOCAL_MODEL.id}"]').click()`);
  ...
  try {
    for (const task of ["hello", "tool", "fix"]) {
      try {
        ...
        const status = await submit(`timeout 180 pi --no-session --mode json -p '/dolly-${task}' > /tmp/studio-${task}.jsonl 2>&1`);
