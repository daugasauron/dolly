...
| `bonnie` | Dolly C frontend plus a retained Python resolver compiled in the Python image and linked to the source-built Fetch-backed libcurl | Greedily resolves recursive pure-Python dependencies from arguments or sequential `-r` requirements files, evaluates and merges markers/extras, accepts only the expected PyPI wheel origin, and downloads/hash/CRC/path-checks the complete chosen graph before touching the target; live acceptance installs Requests, Pytest, and Black, each validated `console_scripts` entry becomes its own `/bin/cc`-compiled wasm64 PATH command, and offline `list`, `freeze`, `show`, and `check` inspect installed metadata and dependency consistency; pip index directives, full backtracking, sdists, and native wheels remain explicit work |
